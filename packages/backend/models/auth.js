...
OAuth2Request.prototype.authByPassword = function(email, pwd) {
  return new Promise((resolve, reject) => {
    (this.db || db).query(
      `select code, password, salt, token, urt.status as tokenStatus
       from user
...
OAuth2Request.prototype.refreshToken = function(token) {
  return new Promise((resolve, reject) => {
    (this.db || db).query(
      `select user_id as userId from user_refresh_token where token='${token}' order by id desc limit 1`,
      (error, results) => {
        if (error) {
          reject(new UnauthorisedError('Unauthorised request.'));
          ...
          (this.db || db).query(
            `select code, salt from user where code='${userId}'`,
            (error, results) => {
              if (error || results.length == 0) {
                reject(new UnauthorisedError('Unauthorised request.'));
              } else {
